...
const suf_log_1 = require("suf-log");
(0, suf_log_1.SetEnvironment)('node');
const colon = (0, suf_log_1.styler)(':', '#777');
// const quote = styler('"', '#f64');
const pipe = (0, suf_log_1.styler)('|', '#f64');
const TEXT = (text) => (0, suf_log_1.styler)(text, '#eee');
const NUMBER = (number) => (0, suf_log_1.styler)(number.toString(), '#f03');
const BOOL = (bool) => (0, suf_log_1.styler)(bool.toString(), bool ? '#4f6' : '#f03');
function LogDebugResult(result) {
    const data = StoreLog.logs;
    let out = (0, suf_log_1.styler)('FORMAT', { "font-weight": 'bold', color: '#0af' });
    for (let i = 0; i < data.length; i++) {
        out += '\n';
        out += InfoLogHelper(data[i]);
    }
    out += `
${pipe}${(0, suf_log_1.styler)(replaceWhitespace(result.replace(/\n/g, '|\n|')), '#c76')}${pipe}`;
    console.log(out);
}
...
class StoreLog {
    static tempConvertData;
    static tempLOCAL_CONTEXT;
    static resetTemp() {
...
function InfoLogHelper(data) {
    ...
    const title = (0, suf_log_1.styler)(info.title, '#cc0');
    ...
    const originalOffset = info.originalOffset !== undefined ? `${TEXT('Original Offset')}${colon} ${NUMBER(info.originalOffset)}` : '';
    const nextLine = info.nextLine !== undefined
        ? JSON.stringify(info.nextLine)
            .replace(/[{}]/g, '')
            .replace(/:/g, ': ')
            .replace(/,/g, ', ')
            .replace(/".*?"/g, (s) => {
            return (0, suf_log_1.styler)(s, '#c76');
        })
        : notProvided;
    const replace = info.replaceSpaceOrTabs !== undefined ? BOOL(info.replaceSpaceOrTabs) : notProvided;
    const CONVERT = convertData
        ? `
      ${TEXT('Convert')}        ${colon} ${(0, suf_log_1.styler)(convertData.type, '#f64')}`
        : '';
    ...
    switch (info.newLineText) {
        ...
        default:
            ...
            if (LOCAL_CONTEXT) {
                data += `\n ${(0, suf_log_1.styler)('LOCAL_CONTEXT', '#f64')} ${(0, suf_log_1.styler)('{', '#777')}`;
                for (const key in LOCAL_CONTEXT) {
                    if (Object.prototype.hasOwnProperty.call(LOCAL_CONTEXT, key)) {
                        const val = LOCAL_CONTEXT[key];
                        data += `\n    ${(0, suf_log_1.styler)(key, '#777')}${colon} ${parseValue(val)}`;
                    }
                }
                data += (0, suf_log_1.styler)('\n }', '#777');
            }
            return ` ${title} ${lineNumber} ${offset} ${originalOffset}
      ${TEXT('Old')}            ${colon} ${(0, suf_log_1.styler)(replaceWhitespace(info.oldLineText), '#d75')}${newText}${CONVERT}${data}`;
    }
}
...
function parseValue(val) {
    ...
    if (type === 'boolean') {
        ...
        return (0, suf_log_1.styler)(val, '#f64');
    }
    else if (type === 'object') {
        return (0, suf_log_1.styler)(JSON.stringify(val), '#0af');
    }
    return val;
}
